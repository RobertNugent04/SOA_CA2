...
export const POST_API = {
  GET_USER_POSTS: (userId: number) =>
    `${API_BASE_URL}/api/posts/user-posts/${userId}`,
...
export const FRIEND_API = {
  ...
  ACCEPT_REQUEST: (friendId: number) =>
    `${API_BASE_URL}/api/friendships/accept/${friendId}`,
};
...
export const MESSAGE_API = {
  GET_CONVERSATION: (friendId: number) =>
    `${API_BASE_URL}/api/messages/conversation/${friendId}`,
